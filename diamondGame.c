...
volatile int * HEX3_ptr = HEX3_HEX0_BASE;
    ...
    *(pixel_ctrl_ptr + 1) = SDRAM_BASE;
    ...
    while (1)   {
        if ((currentPos[0] != position[0] || currentPos[1] != position[1]) || drawn!=2) {
            ...
            drawAllDiamonds();
            diamondSelection(position[1],position[0], diamonds[position[1]][position[0]], 2);
...
void pushbutton_ISR( void ) {
    ...
    *(KEY_ptr + 3) = press; // Clear the interrupt
    ...
    if (SW_value == 0) {
        ...
        else if (press & 0x2) {// KEY1 - left
            if (position[0] > 0) position[0] -= 1;
        }
        else if (press & 0x4) {// KEY2 - down
            if (position[1] < 9) position[1] += 1;
        }
        else if (press & 0x8){// press & 0x8, which is KEY3 up
           if (position[1] > 0)  position[1] -= 1;
        }
    }
    else {
        int row1 = position[1], row2 = position[1], col1 = position[0], col2 = position[0]; 
        if (press & 0x1) {// KEY0 - right 
            if (position[0] < 9) {
                swapValue(position[1], position[1], position[0], position[0]+1);
                col2 = position[0]+1; 
            }
        }
            ...
            if (position[1] < 9) {
                swapValue(position[1], position[1]+1, position[0], position[0]);
                row2 = position[1]+1; 
            }
        }
    ...
    if(row1 == row2 && row1!=0){
        for(i = 1; i < WIDTH;i+=SPEED){
            ...
            if(init >= 2){
                for(b = 0; b < row2;b++){
                    for(a = col1; a <= col2; a++){
                        diamondSelection_XY(convertCol(a),convertRow(b)+i-2*SPEED,CLEAR);
                    }
                }
            }
            ...
            for(b = 0; b < row2;b++){
                for(a = col1; a <= col2; a++){
                    diamondSelection_XY(convertCol(a),convertRow(b)+i,diamonds[b][a]);
                }
            }
            wait_for_vsync(); // swap front and back buffers on VGA vertical sync
            pixel_buffer_start = *(pixel_ctrl_ptr + 1); // new back buffer
...
void clearBlock(int row, int col,int colour){
    int y, x;
        for (y = row-WIDTH/2+1;y <= row + WIDTH/2-1; y++)
                for (x = col-WIDTH/2+1; x <= col + WIDTH/2-1; x++)
    ...
    if(col1 == col2){
        ...
        while(row2 >= 0){
            if(row2 - numOfClear >= 0){
                ...
                diamondSelection(row2, col1, diamonds[row2][col1],1);
                wait_for_vsync(); // swap front and back buffers on VGA vertical sync
                pixel_buffer_start = *(pixel_ctrl_ptr + 1); // new back buffer
                diamondSelection(row2, col1, diamonds[row2][col1],1);
            }else{
        ...
        while(row1 >= 0){
            for(i = col1; i <= col2;i++){
                if(row1-1 >= 0){
                    ...
                    diamondSelection(row1, i, diamonds[row1][i],1);
                    wait_for_vsync(); // swap front and back buffers on VGA vertical sync
                    pixel_buffer_start = *(pixel_ctrl_ptr + 1); // new back buffer
                    diamondSelection(row1, i, diamonds[row1][i],1);
                }else{  
...
void diamondSelection(int row, int col, int num, int color){
    ...
    if(num == 0){
        ...
        drawVerticalSqaure(row, col,ORANGE);
    }else if(num == 3){
        drawVerticalSqaure(row,col, YELLOW);
    }else if(num == 4){
        drawTraingle(row,col,CYAN);
    }else if(num == 5){
...
void drawCrushLine_V(int row1, int row2, int col1, int col2){
    draw_line_2(convertCol(col1), convertCol(col2), convertRow(row1)-1, convertRow(row2)+1, WHITE);
    draw_line_2(convertCol(col1)-1, convertCol(col2)-1, convertRow(row1)-1, convertRow(row2)+1, WHITE);
    draw_line_2(convertCol(col1)+1, convertCol(col2)+1, convertRow(row1)-1, convertRow(row2)+1, WHITE);
    wait_for_vsync(); // swap front and back buffers on VGA vertical sync
    pixel_buffer_start = *(pixel_ctrl_ptr + 1); // new back buffer
    draw_line_2(convertCol(col1), convertCol(col2), convertRow(row1)-1, convertRow(row2)+1, WHITE);
...
void drawCrushLine_H(int row1, int row2, int col1, int col2){
    draw_line_2(convertCol(col1)-1, convertCol(col2)+1, convertRow(row1)-1, convertRow(row2)-1, WHITE);
    draw_line_2(convertCol(col1)-1, convertCol(col2)+1, convertRow(row1), convertRow(row2), WHITE);
    draw_line_2(convertCol(col1)-1, convertCol(col2)+1, convertRow(row1)+1, convertRow(row2)+1, WHITE);
    wait_for_vsync(); // swap front and back buffers on VGA vertical sync
    pixel_buffer_start = *(pixel_ctrl_ptr + 1); // new back buffer
    draw_line_2(convertCol(col1)-1, convertCol(col2)+1, convertRow(row1)-1, convertRow(row2)-1, WHITE);
